...
from dateutil.relativedelta import (
    relativedelta,
)  # relativedelta takes care of leap year
...
def main():
    ...
    while i_pref != 3:
        try:
            i_pref = int(
                input(
                    """Choose the time frame to analyse transactions:
1) Last 5 Years
...
Please choose 1, 2 or 3\n"""
            ...
            if i_pref == 1:
                ...
                print(data)
                print(
                    "Here are the data you requested from {} to {}.".format(
                ...
                while True:
                    try:
                        ...
                        cmin = input(
                            "Choose your start date in YYYY-MM-DD format (Earliest Transaction = {})".format(
                                min_date
                            )
                        ).strip()
                        cmax = input(
                            "Choose your end date in YYYY-MM-DD format (Last Transaction = {})".format(
                                max_date
                            )
                        ).strip()
                        ...
                        if cmax < cmin:
                            ...
                            data = clean_csv[
                                (clean_csv.Date >= cmin) & (clean_csv.Date <= cmax)
                            ]
                            print(data)
                            print(
                                "Here are the data you requested from {} to {}.".format(
                        ...
                        print(
                            "Please input only valid years between {} to {}".format(
                                min_date, max_date
                            )
                        )
                        continue
        ...
        # Exception handling
            ...
            print(
                "I do not understand your input, please try again. Choose 1, 2 or 3 only."
            )
            continue
        break
...
def cluster_choice():
    while True:
        try:
            choice = int(
                input(
                    """Do you want to view the cluster analysis with Customer Code?
1) Yes
2) No
Input here-> """
                )
...
{}""".format(
        ...
        if g_choice2 == 1:
            df["Total Cost of Goods Sold (S$)"] = abs(
                df["Total Cost of Goods Sold (S$)"]
            )

            sns.lineplot(x="Year", y="Total Revenue (S$)", data=df, marker="8")

            sns.lineplot(
                x="Year", y="Total Cost of Goods Sold (S$)", data=df, marker="8"
            )
            ...
            plt.suptitle(
                "Financial Overview by Year: {} to {}".format(
        ...
        dataset = data.copy()
        df = (
            dataset.groupby([dataset.Year, dataset.Month])["Amt", "Worth"]
            .agg("sum")
            .round(2)
        )
        df["Total Profit (S$)"] = df["Amt"] - abs(df["Worth"])
        ...
        df["Year-Month"] = [
            str(df.index[i][0]) + "-" + str(df.index[i][1]) for i in range(len(df))
        ]
        df = df[
            [
                "Year-Month",
...
{}""".format(
        ...
        if g_choice2 == 1:
            ...
            sns.lineplot(
                x=np.arange(len(df)), y="Total Revenue (S$)", data=df, marker="8"
            ).set_xticklabels(df["Year-Month"])
            ...
            sns.lineplot(
                x=np.arange(len(df)), y="Total Profit (S$)", data=df, marker="8"
            ).set_xticklabels(df["Year-Month"])
            ...
            plt.suptitle(
                "Financial Overview by Year-Month: {} to {}".format(
                    min(dataset.Date), max(dataset.Date)
                ),
                fontsize=13,
            )
...
def cust_summary():
    ...
    if choice2 == 1:
        dataset = data.copy()
        # Number of Customers
        df = dataset.groupby(dataset.Year)["CustomerCode"].agg(
            {"Unique Customers": "nunique"}
        )

        # Number of Transaction Type
        df2 = (
            dataset.groupby([dataset.Year, dataset.Type])["Type"]
            .agg({"Count": "count"})
            .reset_index()
        )
        ...
        # Number of Cur Type
        df5 = (
            dataset.groupby([dataset.Year, dataset.Cur])["Cur"]
            .agg({"Count": "count"})
            .reset_index()
        )
...
{}""".format(
        ...
        # Number of Customers
        df = dataset.groupby([dataset.Year, dataset.Month])["CustomerCode"].agg(
            {"Unique Customers": "nunique"}
        )
        df["Year-Month"] = [
            str(df.index[i][0]) + "-" + str(df.index[i][1]) for i in range(len(df))
        ...
        # Number of Transaction Type
        df2 = dataset.groupby([dataset.Year, dataset.Month, dataset.Type])["Type"].agg(
            {"Count": "count"}
        )
        df2["Year-Month"] = [
            str(df2.index[i][0]) + "-" + str(df2.index[i][1]) for i in range(len(df2))
        ]
        ...
        # Number of Cur Type
        df5 = dataset.groupby([dataset.Year, dataset.Month, dataset.Cur])["Cur"].agg(
            {"Count": "count"}
        )
        df5["Year-Month"] = [
            str(df5.index[i][0]) + "-" + str(df5.index[i][1]) for i in range(len(df5))
        ]
...
======================================="""
        ...
        print(
            """Frequency of Transactions by Currency
{}""".format(
                No_TransCur
            )
        )
...
def transaction_trend():
    ...
    if choice2 == 1:
        ...
        for each in list_of_data:
            ...
            mode_NoTrans = str(
                list(each.groupby(each.CustomerCode).size().mode())
            ).strip("[]")
            ...
            each_Quantity = each_Quantity[
                each_Quantity.replace([np.inf, -np.inf], np.nan).notnull().all(axis=1)
            ]
            NoTrans_Qty = pd.merge(
                NoTrans_Qty, each_Quantity, on="CustomerCode", how="left"
            )
...
===============================================================================================================""".format(
        ...
        if g_choice2 == 1:
            ...
            ax1.set_title(
                "Customer Number of Transactions across the Years: {} to {}".format(
                    min(dataset.Date), max(dataset.Date)
                ),
                fontsize=9,
            )
            ...
            sns.lineplot(
                x="Year", y="Median NoT", data=df, marker="8", ax=ax2, color="tomato"
            )
            ax2.set_ylabel("Median Number of Transactions", fontsize=7)
            ax2.set_title(
                "The Average Customer's Number of Transactions across the Years: {} to {}".format(
                    min(dataset.Date), max(dataset.Date)
                ),
                fontsize=9,
            )
            ...
            ax3.set_title(
                "The Average Customer's Transaction Size across the Years: {} to {}".format(
                    min(dataset.Date), max(dataset.Date)
                ),
                fontsize=9,
            )
...
===================================================================================================================""".format(
        ...
        if g_choice2 == 1:
            ...
            sns.lineplot(
                x=np.arange(len(df)), y="NoT", data=df, marker="8", ax=ax1
            ).set_xticklabels(df["Year-Month"])
            ax1.set_ylabel("Number of Transactions", fontsize=7)
            ax1.set_title(
                "Customer Number of Transactions across the Year-Months: {} to {}".format(
                    min(dataset.Date), max(dataset.Date)
                ),
                fontsize=9,
            )
            ...
            ax2.set_ylabel("Median Number of Transactions", fontsize=7)
            ax2.set_title(
                "The Average Customer's Number of Transactions across the Year-Months: {} to {}".format(
                    min(dataset.Date), max(dataset.Date)
                ),
                fontsize=9,
            )
            ...
            ax3.set_ylabel("Median Quantity Purchased per StockISN", fontsize=7)
            ax3.set_title(
                "The Average Customer's Transaction Size across the Year-Months: {} to {}".format(
                    min(dataset.Date), max(dataset.Date)
                ),
                fontsize=9,
            )
...
Mode Number of Transactions: {}""".format(
                mean_NoTrans, median_NoTrans, mode_NoTrans
            )
        )
        print("===============================================")
        returnmain()
...
def purchase_trend():
    ...
    if choice2 == 1:
        ...
        for dataset in list_of_data:
            ...
            # Quantity Purchased
            dataset_Quantity = pd.DataFrame(
                dataset.groupby("CustomerCode").Quantity.sum()
            )
            dataset_Quantity = dataset_Quantity[
                dataset_Quantity.replace([np.inf, -np.inf], np.nan)
                .notnull()
                .all(axis=1)
            ]
            ...
            median_NoStkType = int(
                dataset.groupby(dataset.CustomerCode).StockISN.nunique().median()
            )
            mode_NoStkType = str(
                list(dataset.groupby(dataset.CustomerCode).StockISN.nunique().mode())
            ).strip("[]")

            # Purchase Variety: Quantity Purchased per StockISN
            NoStkType_Qty = (
                dataset.groupby("CustomerCode").StockISN.nunique().to_frame()
            )
            ...
            NoStkType_Qty = pd.merge(
                NoStkType_Qty, dataset_Quantity, on="CustomerCode", how="left"
            )
            NoStkType_Qty["PurchaseVariety"] = (
                NoStkType_Qty["Quantity"] / NoStkType_Qty["NoStkType"]
            )
...
=====================================================================================================================""".format(
        ...
        if g_choice2 == 1:
            ...
            df["Total Number of Unique StockISN"] = df[
                "Total Number of Unique StockISN"
            ].astype(int)
            ...
            sns.lineplot(
                x="Year", y="Total Quantity Purchased", data=df, marker="8", ax=ax1
            )
            ax1.set_ylabel("Total Quantity Purchased", fontsize=6)
            ax1.set_title(
                "Total Quantity Customer Purchased across the Years: {} to {}".format(
                    min(dataset1.Date), max(dataset1.Date)
                ),
                fontsize=9,
            )
            ...
            ax2.set_title(
                "The Average Customer's Quantity Purchased across the Years: {} to {}".format(
                    min(dataset1.Date), max(dataset1.Date)
                ),
                fontsize=9,
            )
            ...
            ax3.set_title(
                "Number of Unique StockISN Customer Purchased across the Years: {} to {}".format(
                    min(dataset1.Date), max(dataset1.Date)
                ),
                fontsize=9,
            )
            ...
            sns.lineplot(
                x="Year", y="Median NoUS", data=df, marker="8", ax=ax4, color="tomato"
            )
            ax4.set_ylabel("Median Number of StockISN", fontsize=6)
            ax4.set_title(
                "The Average Customer's Number of StockISN Purchased across the Years: {} to {}".format(
                    min(dataset1.Date), max(dataset1.Date)
                ),
                fontsize=9,
            )
            ...
            ax5.set_title(
                "The Average Customer's Purchase Variety across the Years: {} to {}".format(
                    min(dataset1.Date), max(dataset1.Date)
                ),
                fontsize=9,
            )
...
===============================================================================================================================""".format(
        ...
        if g_choice2 == 1:
            ...
            sns.lineplot(
                x=np.arange(len(df)), y="Median QP", data=df, marker="8", ax=ax2
            ).set_xticklabels(df["Year-Month"])
...
==============================================="""
        ...
        dataset = dataset[
            dataset.replace([np.inf, -np.inf], np.nan).notnull().all(axis=1)
        ]
        total_Qty = int(dataset["Quantity"].sum())
        median_Qty = int(
            dataset.groupby(dataset.CustomerCode)["Quantity"].sum().median()
        )
        ...
        mode_NoUS = list(
            dataset.groupby(dataset.CustomerCode).StockISN.nunique().mode()
        )
...
Mode of Quantity Purchased: {}""".format(
                median_Qty, mode_Qty
            )
        )
        print("-----------------------------------------------")
...
Mode Number of Unique Stock Purchased: {}""".format(
                median_NoUS, mode_NoUS
            )
        )
        print("-----------------------------------------------")
        print(
...
Mode Number of Unique Stock Purchased: {}""".format(
                median_PurchaseVariety, mode_PurchaseVariety
            )
        )
        returnmain()
...
def financial_trend():
    ...
    if choice2 == 1:
        ...
        for dataset in list_of_data:
            ...
            # Revenue
            median_Rev = round(
                dataset.groupby(dataset.CustomerCode).Amt.sum().median(), 2
            )

            # Cost
            median_Cost = round(
                dataset.groupby(dataset.CustomerCode).Worth.sum().median(), 2
            )

            # Profit
            median_Profit = round(
                dataset.groupby(dataset.CustomerCode).Profit.sum().median(), 2
            )
...
==========================================================================""".format(
        ...
        if g_choice2 == 1:
            ...
            df["Median Cost of Goods Sold"] = abs(
                df["Median Cost of Goods Sold"].astype(float)
            )
            ...
            ax1.set_title(
                "The Average Customer's Revenue across the Years: {} to {}".format(
                    min(dataset1.Date), max(dataset1.Date)
                ),
                fontsize=10,
            )
            ...
            ax2.set_title(
                "The Average Customer's Cost of Goods Sold across the Years: {} to {}".format(
                    min(dataset1.Date), max(dataset1.Date)
                ),
                fontsize=10,
            )
            ...
            ax3.set_title(
                "The Average Customer's Profit across the Years: {} to {}".format(
                    min(dataset1.Date), max(dataset1.Date)
                ),
                fontsize=10,
            )
        ...
        for dataset in list_of_data:
            ...
            # Revenue
            median_Rev = (
                dataset.groupby(dataset.CustomerCode).Amt.sum().median().round(2)
            )

            # Cost
            median_Cost = (
                dataset.groupby(dataset.CustomerCode).Worth.sum().median().round(2)
            )

            # Profit
            median_Profit = (
                dataset.groupby(dataset.CustomerCode).Profit.sum().median().round(2)
            )
...
==========================================================================""".format(
        ...
        if g_choice2 == 1:
            ...
            sns.lineplot(
                x=np.arange(len(df)), y="Median Revenue", data=df, marker="8", ax=ax1
            ).set_xticklabels(df["Year-Month"])

            ax1.set_ylabel("Median Revenue", fontsize=9)
            ax1.set_xlabel("Year-Month", fontsize=9)
            ax1.set_title(
                "The Average Customer's Revenue across the Year-Months: {} to {}".format(
                    min(dataset1.Date), max(dataset1.Date)
                ),
                fontsize=10,
            )
            ...
            ax2.set_title(
                "The Average Customer's Cost of Goods Sold across the Year-Months: {} to {}".format(
                    min(dataset1.Date), max(dataset1.Date)
                ),
                fontsize=10,
            )
            ...
            ax3.set_title(
                "The Average Customer's Profit across the Year-Month: {} to {}".format(
                    min(dataset1.Date), max(dataset1.Date)
                ),
                fontsize=10,
            )
...
==============================================="""
        ...
        mode_Rev = str(
            list(dataset.groupby(dataset.CustomerCode).Amt.sum().mode().round(2))
        )
        ...
        mode_Cost = str(
            list(dataset.groupby(dataset.CustomerCode).Worth.sum().mode().round(2))
        )
        ...
        mode_Profit = str(
            list(dataset.groupby(dataset.CustomerCode).Profit.sum().mode().round(2))
        )
...
Mode Revenue: {}""".format(
        ...
        print(
            """Across the Customers,
Median Cost of Goods Sold = {}
Mode Cost of Goods Sold: {}""".format(
                median_Cost, mode_Cost
            )
        )
        print("-----------------------------------------------")
        print(
...
def cluster_analysis():
    ...
    fig.suptitle(
        "Customer Profit-Quantity Cluster Analysis across the Years: {} to {}".format(
            min(dataset.Date), max(dataset.Date)
        ),
        fontsize=12,
    )
    ...
    for i in range(num_of_plots + 1):
        ...
        cust_profit = (
            dataset.loc[dataset.Year == start].groupby("CustomerCode").Profit.sum()
        )
        cust_profit = cust_profit[cust_profit != 0]
        cust_profit.head()

        cust_quantity = (
            dataset.loc[dataset.Year == start].groupby("CustomerCode").Quantity.sum()
        )
        cust_quantity = cust_quantity[cust_quantity != 0]
        cust_quantity.head()

        # Create a dataframe combining both profit and quantity
        profit_quantity = pd.DataFrame(
            {"Profit": cust_profit, "Quantity": cust_quantity}
        )
        ...
        profit_quantity = profit_quantity[
            profit_quantity.replace([np.inf, -np.inf], np.nan).notnull().all(axis=1)
        ]
        profit_quantity
        ...
        if c_choice == 1:
            ...
            for i, txt in enumerate(profit_quantity.CustomerCode):
                ax[row, col].annotate(
                    txt, (profit_quantity.Profit[i], profit_quantity.Quantity[i])
                )
...
def LeastprofitableStocks():
    ...
    stock_profit32 = stock_profit32[
        stock_profit32["Quantity"].replace([np.inf, -np.inf], np.nan).notnull()
    ]
    while True:
        try:
            choice = int(
...
Input here-> """
            ...
            if choice == 1:
                stock_profit32 = stock_profit32.groupby(
                    ["StockISN", "Year"], as_index=False
                )[["Quantity", "Profit"]].sum()
                for i in stock_profit32.Year.unique():
                    print("For Year {} the bottom 5 stocks are:".format(i))
                    print(
                        stock_profit32[stock_profit32.Year == i].nsmallest(5, "Profit")
                    )
                break
            elif choice == 2:
                stock_profit32 = stock_profit32.groupby(
                    ["StockISN", "Month"], as_index=False
                )[["Quantity", "Profit"]].sum()
                for i in stock_profit32.Month.unique():
                    print("For Month {} the top 5 stocks are:".format(i))
                    print(
                        stock_profit32[stock_profit32.Month == i].nsmallest(5, "Profit")
                    )
                break
            elif choice == 3:
                stock_profit32 = stock_profit32.groupby("StockISN", as_index=False)[
                    ["Quantity", "Profit"]
                ].sum()
...
def topfivesoldstocks():
    ...
    stock_isn3 = stock_isn3[
        stock_isn3["Profit"].replace([np.inf, -np.inf], np.nan).notnull()
    ]
    while True:
        try:
            choice = int(
...
Input here-> """
            ...
            if choice == 1:
                ...
                fig.set_size_inches(5.5, 3)
                fig.tight_layout(w_pad=1.5, h_pad=1.5)
                stock_isn3 = stock_isn3.groupby(["StockISN", "Year"], as_index=False)[
                    ["Quantity", "Profit"]
                ].sum()
                a = 0
                for i in stock_isn3.Year.unique():
                    ...
                    row, col = int(a / 3), a % 3
                    ax[row, col].bar(
                        tempprint.StockISN, tempprint.Quantity, color="#00BFC4"
                    )
                ...
                stock_isn3 = stock_isn3.groupby(["StockISN", "Month"], as_index=False)[
                    ["Quantity", "Profit"]
                ].sum()
                ...
                stock_isn3 = stock_isn3.groupby("StockISN", as_index=False)[
                    ["Quantity", "Profit"]
                ].sum()
                print("The top 10 most sold items in the timeframe is:")
...
def topfiveprofitstocks():
    ...
    stock_profit3 = stock_profit3[
        stock_profit3["Quantity"].replace([np.inf, -np.inf], np.nan).notnull()
    ]
    while True:
        try:
            choice = int(
...
Input here-> """
            ...
            if choice == 1:
                stock_profit3 = stock_profit3.groupby(
                    ["StockISN", "Year"], as_index=False
                )[["Quantity", "Profit"]].sum()
                ...
                stock_profit3 = stock_profit3.groupby("StockISN", as_index=False)[
                    ["Quantity", "Profit"]
                ].sum()
                print("The top 10 most sold items in the timeframe is:")
...
def regional_analysis():
    ...
    while True:
        try:
            regional = regional_data.loc[
                :, ["CustomerRegion", "Year", "Month", "Profit", "Quantity"]
            ].sort_values(["CustomerRegion", "Year", "Month"])
...
Input -> """
            ...
            if choice == 1:
                rp_cust_reg_year = regional.groupby(["CustomerRegion", "Year"])[
                    ["Profit", "Quantity"]
                ].sum()
                ...
                rp_cust_reg_ymonth = regional.groupby(
                ...
                rp_cust_reg = regional.groupby("CustomerRegion")[
                    ["Profit", "Quantity"]
                ].sum()
